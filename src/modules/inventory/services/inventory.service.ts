...
export class InventoryService {
  ...
  async updateStock(
    ...
    const quantityChange = updateStockDto.quantity;
    ...
    switch (updateStockDto.type) {
      ...
      case StockUpdateType.RESERVE:
        if (inventory.quantity - quantityChange < 0) {
          throw new BadRequestException(
            'Cannot reserve more items than available',
          );
        }
        inventory.quantity -= quantityChange;
      ...
      case StockUpdateType.RELEASE:
        if (inventory.reservedQuantity - quantityChange < 0) {
          throw new BadRequestException(
            'Cannot release more items than reserved',
          );
        }
        inventory.quantity += quantityChange;
        inventory.reservedQuantity -= quantityChange;
    ...
    inventory.stockMovements = [
      ...(inventory.stockMovements || []),
      {
        ...
        reference:
          ...
          `${updateStockDto.type.toUpperCase()}_${Date.now()}`,
      },
    ];
  ...
  async getInventoryStats(): Promise<{
    ...
    return {
      ...
      outOfStockProducts: inventories.filter(
