...
export class BulkOrderService {
  ...
  async create(
    ...
    bulkOrder.bulkDiscountPercentage = this.calculateBulkDiscount(
      bulkOrder.totalAmount,
      items,
    );
    bulkOrder.finalAmount =
      bulkOrder.totalAmount * (1 - bulkOrder.bulkDiscountPercentage / 100);
  ...
  async updateStatus(
    ...
    if (
      oldStatus !== BulkOrderStatus.PROCESSING &&
      status === BulkOrderStatus.PROCESSING
    ) {
  ...
  async getBulkOrderSummary(userId: string, startDate: Date, endDate: Date) {
    ...
    const totalDiscount = orders.reduce(
      (sum, order) =>
        sum + (Number(order.totalAmount) - Number(order.finalAmount)),
      0,
    );
