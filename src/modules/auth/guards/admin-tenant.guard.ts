...
export class AdminTenantGuard extends JwtAuthGuard {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        const guards = this.reflector.getAllAndMerge<any[]>('__guards__', [
            context.getHandler(),
            context.getClass(),
        ]) || [];

        const hasJwtGuard = guards.some(
            (guard) => guard === JwtAuthGuard || (guard && guard.name === 'JwtAuthGuard')
        );
        ...
        if (resolvedTenantId && user.tenantId && user.tenantId !== resolvedTenantId) {
            throw new ForbiddenException('Access Denied: You do not belong to this tenant.');
        }
