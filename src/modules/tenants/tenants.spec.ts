...
describe('Tenants Module Unit Tests', () => {
    describe('TenantMiddleware', () => {
            ...
            mockTenantsService = {
                findBySlug: jest.fn().mockImplementation(async (slug) => {
                    if (slug === 'gadgets-heaven') {
                        return { id: 'default-id', name: 'Gadgets Heaven', slug: 'gadgets-heaven' };
                    }
                    if (slug === 'jersey-mania') {
                        return { id: 'jersey-id', name: 'Jersey Mania', slug: 'jersey-mania' };
                    }
                    return null;
                }),
        ...
        it('should resolve tenant from header x-tenant-slug', async () => {
            ...
            expect(mockTenantsService.findBySlug).toHaveBeenCalledWith('jersey-mania');
        ...
        it('should fallback to gadgets-heaven if no slug or subdomain is provided', async () => {
            ...
            expect(mockTenantsService.findBySlug).toHaveBeenCalledWith('gadgets-heaven');
        ...
        describe('afterLoad', () => {
            ...
            it('should throw ForbiddenException if entity tenantId does not match context tenantId', () => {
                tenantStorage.run({ tenantId: 'tenant-a', slug: 'tenant-a' }, () => {
                    ...
                    expect(() => subscriber.afterLoad(entity)).toThrow(ForbiddenException);
                });
            });
        ...
        describe('beforeUpdate', () => {
            ...
            it('should throw ForbiddenException if updating entity tenantId mismatch', () => {
                tenantStorage.run({ tenantId: 'tenant-a', slug: 'tenant-a' }, () => {
                    ...
                    expect(() => subscriber.beforeUpdate(event)).toThrow(ForbiddenException);
                });
            });
    ...
    describe('AdminTenantGuard', () => {
        ...
        beforeEach(() => {
            ...
            jest.spyOn(guard as any, 'canActivate').mockImplementation(async (context: ExecutionContext) => {
                const request = context.switchToHttp().getRequest();
                const guards = mockReflector.getAllAndMerge('__guards__', [
                    context.getHandler(),
                    context.getClass(),
                ]) || [];
                ...
                if (resolvedTenantId && user.tenantId && user.tenantId !== resolvedTenantId) {
        ...
        it('should reject requests if tenantId in request user does not match context tenantId', async () => {
            ...
            await tenantStorage.run({ tenantId: 'tenant-b', slug: 'tenant-b' }, async () => {
        ...
        it('should allow requests if tenantId matches context tenantId', async () => {
            ...
            await tenantStorage.run({ tenantId: 'tenant-a', slug: 'tenant-a' }, async () => {
                const result = await guard.canActivate(mockContext);
                expect(result).toBe(true);
            });
        });
