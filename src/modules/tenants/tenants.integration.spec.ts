...
describe('Tenants Integration Proof (Real DB)', () => {
  ...
  it('should isolate data during querying (Tenant B cannot see Tenant A)', async () => {
    const jerseyCategory = await tenantStorage.run(
    ...
    await tenantStorage.run(
      ...
      async () => {
        await expect(
          categoryRepository.findOne({ where: { id: jerseyCategory!.id } }),
  ...
  it('should isolate data during querying (Tenant A cannot see Tenant B)', async () => {
    const jerseyProduct = await tenantStorage.run(
      { tenantId: SECOND_TENANT_ID, slug: 'jersey-mania' },
      async () => {
    ...
    await tenantStorage.run(
      ...
      async () => {
        await expect(
          productRepository.findOne({ where: { id: jerseyProduct!.id } }),
        ).rejects.toThrow(ForbiddenException);
      },
    );
  });
  ...
  it('should reject cross-tenant access to orders, reviews, users, and finance endpoints', async () => {
    ...
    for (const endpoint of endpoints) {
      ...
      await req.expect((res) => {
        if (res.status !== 401 && res.status !== 403) {
          throw new Error(
            `Expected 401 or 403, got ${res.status} on ${endpoint.path}`,
          );
        }
      });
    }
